...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.loanMemfeeSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.loan-memfee-schedules.update", [$loanMemfeeSchedule->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shareholder') ? 'is-invalid' : '' }}" name="shareholder_id" id="shareholder_id" required>
                    @foreach($shareholders as $id => $shareholder)
                        <option value="{{ $id }}" {{ ($loanMemfeeSchedule->shareholder ? $loanMemfeeSchedule->shareholder->id : old('shareholder_id')) == $id ? 'selected' : '' }}>{{ $shareholder }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('loan') ? 'is-invalid' : '' }}" name="loan_id" id="loan_id" required>
                    @foreach($loans as $id => $loan)
                        <option value="{{ $id }}" {{ ($loanMemfeeSchedule->loan ? $loanMemfeeSchedule->loan->id : old('loan_id')) == $id ? 'selected' : '' }}>{{ $loan }}</option>
                    @endforeach
                </select>
                @if($errors->has('loan'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_plan') ? 'is-invalid' : '' }}" type="text" name="date_plan" id="date_plan" value="{{ old('date_plan', $loanMemfeeSchedule->date_plan) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mem_fee_plan') ? 'is-invalid' : '' }}" type="number" name="mem_fee_plan" id="mem_fee_plan" value="{{ old('mem_fee_plan', $loanMemfeeSchedule->mem_fee_plan) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mem_fee_fact') ? 'is-invalid' : '' }}" type="number" name="mem_fee_fact" id="mem_fee_fact" value="{{ old('mem_fee_fact', $loanMemfeeSchedule->mem_fee_fact) }}" step="0.01" required>
